...
export default function DeliveryPointDetail() {
  ...
  async function handleDelete() {
    if (!window.confirm("Delete this delivery point permanently?")) return;
    setBusy(true);
    try {
      await DeliveryPointsAPI.remove(id, deletePhone.trim());
      navigate("/");
    } catch (err) {
      showToast(err.response?.data?.error || err.message || "Could not delete delivery point.", "error");
    } finally {
      setBusy(false);
    }
  }
        ...
        <div className="grid grid-2" style={{ alignItems: "start" }}>
          ...
          <div>
            ...
            <div className="card delete-panel" style={{ marginTop: 20 }}>
              ...
              <p className="hint">Enter the phone number used when this delivery point was created.</p>
              {!showDeleteForm ? (
                <button className="btn btn-danger-outline btn-block" onClick={() => setShowDeleteForm(true)}>
                  Delete delivery point
                </button>
              ) : (
                <div>
                  <div className="field">
                    <label htmlFor="delete-phone">Your phone with country code</label>
                    <input id="delete-phone" type="tel" placeholder="e.g. +919876543210" value={deletePhone} onChange={(e) => setDeletePhone(e.target.value)} pattern="\\+[1-9][0-9]{7,14}" required />
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button className="btn btn-outline" style={{ flex: 1 }} onClick={() => setShowDeleteForm(false)}>Cancel</button>
                    <button className="btn btn-danger-outline" style={{ flex: 1 }} onClick={handleDelete} disabled={busy || !deletePhone.trim()}>Delete</button>
                  </div>
                </div>
              )}
            </div>
