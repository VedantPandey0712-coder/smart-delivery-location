...
function deleteLocal(id, customerPhone) {
  const point = localPoints().find((item) => item.id === id);
  if (!point || point.customerPhone !== customerPhone) {
    throw new Error("Delivery point not found or phone number does not match.");
  }
  saveLocalPoints(localPoints().filter((item) => item.id !== id));
}
...
export const DeliveryPointsAPI = {
  ...
  remove: (id, customerPhone) => withLocalFallback(() => api.delete(`/delivery-points/${id}`, { data: { customerPhone } }).then(() => undefined), () => deleteLocal(id, customerPhone)),
  uploadPhoto: (id, file) => {
