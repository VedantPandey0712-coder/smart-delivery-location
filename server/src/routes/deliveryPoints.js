...
// Create a new delivery point (Step 1: address text and required customer info)
router.post("/", async (req, res, next) => {
  try {
    ...
    if (typeof customerPhone !== "string" || !E164_PHONE_REGEX.test(customerPhone.trim())) {
      return res.status(400).json({ error: "customerPhone must include a valid country code, e.g. +919876543210." });
    }
    ...
    try {
      const { rows } = await client.query(
        ...
        [addressText.trim(), customerName.trim(), customerPhone.trim()]
      );
      const point = rows[0];
...
// Delete a delivery point after verifying the owner's phone number.
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    const { rowCount } = await pool.query(
      `DELETE FROM delivery_points WHERE id = $1 AND customer_phone = $2`,
      [req.params.id, customerPhone.trim()]
    );
    if (!rowCount) return res.status(404).json({ error: "Delivery point not found or phone number does not match." });
    res.status(204).send();
  } catch (err) {
    next(err);
  }
